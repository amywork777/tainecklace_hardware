...
const uint32_t LONG_PRESS_DELAY_MS = 2000;
...
void setup(){
  ...
  Serial.println("  r / D0 press = start/stop recording");
  Serial.println("  s / D0 press = stop recording");
  Serial.println("  c / D1 hold 2s = connect BLE transfer");
  Serial.println("  l / D2 press = list files");
  Serial.println("  d / D2 hold 2s = delete all WAV files");
...
void loop(){
  ...
  if (update_button_state(&btn_record, BTN_RECORD_PIN)) {
    // D0: Toggle recording (start if stopped, stop if recording)
    if (audio_is_recording()) {
      Serial.println("> Button: Stop recording");
      ...
      Serial.println("> Button: Start recording");
  ...
  if (update_button_state(&btn_transfer, BTN_TRANSFER_PIN)) {
